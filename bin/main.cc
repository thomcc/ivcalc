...
void run_benchmark(PartialCalc &pc) {
...
void parse_cmd(string const &src, bool &verbose, bool &benchmark, bool &codegen, bool &partials) {
	switch(src[1]) {
	case 'b':
		benchmark = !benchmark;
...
int handle_expr(ExprPtr const&, bool, bool, bool, bool, Evaluator&, Compiler&, Printer&);

int repl(bool verbose, bool benchmark, bool codegen, bool emit_partials) {
	...
	for (;;) {
		...
		for (;;) {
			...
			ltrim(src);
			if (src[0] == '!') {
				parse_cmd(src, verbose, benchmark, codegen, emit_partials);
				src = "";
				continue;
			}
		...
		int ret = handle_expr(expr, verbose, benchmark, codegen, emit_partials, e, c, print);
		if (ret) return ret;
	}
}

int handle_expr(string const &expr_src, bool vb, bool bm, bool cg, bool part) {
	...
	return handle_expr(expr, vb, bm, cg, part, e, c, print);
}
	...
	if (expr.get()) {
		...
		if (benchmark) {
			...
			if (FuncExpr const *fe = expr->as_func_expr()) {
				...
				if (verbose) cout << "not benchmarking non-function." << endl;
				return 0;
			}
...
Args::Args(int argc, char **argv) {
	...
	struct { bool v, b, c, p, r, e, f, s; } flags = { 0 };
...
repl_flag:
		...
		case '-':
			if (strcmp((*argv)+2, "bench") == 0) {
				...
				goto codegen_flag;
			else if (strcmp((*argv)+2, "emit-partials") == 0)
				goto partials_flag;
			else if (strcmp((*argv)+2, "repl") == 0)
				goto repl_flag;
...
int main(int argc, char *argv[]) {
	...
	if (args.from_str) return handle_expr(args.e_expr, args.verbose, args.benchmark, args.compile, args.compile_partials);
	else return repl(args.verbose, args.benchmark, args.compile, args.compile_partials);
}
