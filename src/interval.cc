...
vec_iv vec_iv_div(vec_iv x, vec_iv y) {
	return vec_iv_mul(x, vec_iv_invert(y));
}


pod_interval iv_vadd(pod_interval x, pod_interval y) {
	vec_iv v = vec_iv_add((vec_iv){x.lo, x.hi}, (vec_iv){y.lo, y.hi});
	return pod_interval{vec_iv_lo(v), vec_iv_hi(v)};
}

pod_interval iv_vsub(pod_interval x, pod_interval y) {
	vec_iv v = vec_iv_sub((vec_iv){x.lo, x.hi}, (vec_iv){y.lo, y.hi});
	return pod_interval{vec_iv_lo(v), vec_iv_hi(v)};
}

pod_interval iv_vmul(pod_interval x, pod_interval y) {
	vec_iv v = vec_iv_mul((vec_iv){x.lo, x.hi}, (vec_iv){y.lo, y.hi});
	return pod_interval{vec_iv_lo(v), vec_iv_hi(v)};
}

pod_interval iv_vdiv(pod_interval x, pod_interval y) {
	vec_iv v = vec_iv_div((vec_iv){x.lo, x.hi}, (vec_iv){y.lo, y.hi});
	return pod_interval{vec_iv_lo(v), vec_iv_hi(v)};
}

pod_interval iv_vneg(pod_interval x) {
	vec_iv v = vec_iv_negate((vec_iv){x.lo, x.hi});
	return pod_interval{vec_iv_lo(v), vec_iv_hi(v)};
}
